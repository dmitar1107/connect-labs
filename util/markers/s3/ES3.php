 ...
 class ES3 extends CApplicationComponent
{
	...
	public function getObjectInfo($uri, $bucket="", $returnInfo = true)
        {
          if($bucket==""){
            $bucket=$this->bucket;
          }
          $s3 = $this->getInstance();
          return $s3->getObjectInfo($uri, $bucket, $returnInfo);
        }
        public function getObject($uri, $bucket="", $saveTo = false) {
          if($bucket==""){
            $bucket=$this->bucket;
          }
        $s3 = $this->getInstance();
